...
KeyDelete.prototype.defaultClickAction = function() {
  ...
  this.id = "mlkeyboard-deleteall";
  this.default_value = 'Borrar todo';
}

KeyDeleteAll.prototype = new Key();
KeyDeleteAll.prototype.constructor = KeyDeleteAll;

KeyDeleteAll.prototype.defaultClickAction = function() {
  this.keyboard.deleteAll();
};
...
KeyEnter.prototype.defaultClickAction = function() {
  this.keyboard.hideKeyboard(); // esconde solamente el teclado el enter para que los archvios exterires hagan la función 
};
...
Keyboard.prototype.renderKeys = function() {
  ...
  for (var i = 0; i<= KEYS_COUNT; i++) {
    ...
    case 28:
      ...
      break;
    case 40:
      // key = new KeyReturn(this);  //ANTES ERA RETURN QUE NO SE SABE PARA QUE SIRVE 
      key = new KeyTab(this);   // AHORA ES TAB
      break;
    case 41:
...
Keyboard.prototype.deleteAll = function() {
  this.$current_input.val('').focus();
};
...
Keyboard.prototype.showModifications = function(caller) {
  ...
  // TODO: Remove magic numbers  ACENTOS
  // width = (caller.$key.width() * _this.modifications.length) + (_this.modifications.length * 6 + (4 * _this.modifications.length)); // la última operacion multiplica la cantidad de teclas por 4px ya que las teclas tiene margen de 4px
...
mlKeyboard.layouts.es_ES = [
  ...
  {},  // deleteAll
  {},  // Space
  {} // Enter, esta tecla no hará nada dentro de este archivo, lo realizará en los otros archivos en donde se de click
];
